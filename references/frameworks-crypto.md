...
- **第一性原理 = 点对点公开透明全球公共账本 (a peer-to-peer, transparent, global public ledger).** Signature accounting-ledger narrative: **三次记账范式变革 (单式 → 复式 → 分布式记账)** and **四次资本范式 (财团 → 华尔街 → 硅谷 → 加密).**
- **Two 胜负手 of Crypto = 资产流量 + 合规 — DO NOT DROP 合规 (a documented miss).** Both are first-class; an L2/L1 or ecosystem report that analyzes only asset flow and drops compliance entirely has missed half the opening thesis. Analyze **合规 at three levels: 牌照层 (license cost) / 商业模式层 (margin impact) / 基因层 (team gene).** Carry compliance into the summary, not just the exchange chapter.
- **资产渗透率 North Star, grounded in the Crypto asset-class MAP.** Anchor "asset penetration" with concrete asset-class sizes (e.g. BTC vs gold ~$720B mapping, stablecoin sizes, NFT ~$1.53B from Rootdata/NFTGo) — size where on-chain assets are vs the global pools they map to, rather than asserting "early."
...
- **Bottom-up ETH revenue WORKSHEET — build the full table, DERIVE the price (`data-analysis.md` 2.20).** **ETH revenue = L1 gas 收入 + L2 上交的数据存储费**; build per-cell, computed SEPARATELY for ETH-only and ETH+L2, in bear/base/bull: `单次交易收入 × 交易次数 × 下降/上升倍数 → 未来收入 → × PS → 市值 → ÷ 供应量 → 价格`. Reproduce every cell, e.g. per-tx revenue $6/$11/$22; decline multiples 50/60/80%; future tx counts 378.4M/612.8M/923.6M; future revenue $1135M/$4044M/$16255M; resulting price ETH-only $1419/$3370/$6773 and ETH+L2 $1444/$3649/$7331. PS bands 熊市 120-150 / 牛市 40-50. **DERIVE the end price from the table** — do not cite a rounder number that diverges from the derived figure (the human's bull is ~$7331, not "$9-10k").
  - **Per-L2 contribution sub-models (required):** each major L2's L1-paid revenue with its tx-count multiplier and per-tx L1 fee — e.g. Arbitrum future L1-paid revenue $10.5M/$131.8M/$395.4M (tx-count 2x/10x/15x, per-tx L1 fee $0.02/$0.05/$0.10); Optimism $5.5M/$102.2M/$409M (2x/15x/30x). State Arb+OP as a % of total L2 contribution to ETH (e.g. 80%/70%/60% bear/base/bull).
- **Gross-margin PROOF for "L2 商业模式 = 云计算" (`data-analysis.md` Part 1 crypto).** Don't just assert the cloud-computing model — cite the **ETH gross-margin time series (~70-90%)** and the **L2 net-revenue-to-L1-remittance ratio over time** (the ratio is shrinking — that is the real value-capture evidence, not a single −95% figure).
- **Competitive landscape with REAL figures (`data-analysis.md` Part 1 crypto + l2beat in data-sources).** Build the per-L2 TVL-share table (Arbitrum One ~49.58% $10.89B, OP Mainnet ~27.38% $6.01B, Base ~3.45%, zkSync ~2.48%…; top-5 Op-Rollup combined ~85%), the gas-cost-per-action table (send ETH 21000 gas, ERC-20 65000, NFT ~84904, Uniswap swap ~184523; gas = units × (base fee + priority fee)), the real cross-chain bench (gas$/real-TPS/TVL/DAU/core-devs for ETH/Arb/OP/BSC/Polygon/Solana — e.g. Solana real TPS ~2398 vs Ethereum ~16.4), and per-chain top-project TVL + Mcap/TVL tables (GMX/Synthetix/Velodrome/Aave V3…; top-3 ~56% of OP TVL). Use REAL dev counts (ETH ~2000; Arb ~37, OP ~68) — never a fabricated "~10:1" ratio.
- **Ethereum upgrade roadmap as the technical backbone of the gas-collapse argument:** the five-stage Merge → Surge → Verge → Purge → Splurge with TPS targets (10-20 now → 100k) and the EIP-4844 → Danksharding L2-cost-reduction ladder (10x → 100x). Tie the −95% fee collapse to this staged cost path, not a bare figure.
- **Technical architecture + team/funding depth (don't stay at framework level):** Arbitrum (Sequencer/Validator flow, 7-day challenge window, Nitro VM, Orbit/Nova/One product line with per-product TVL/TPS/gas; Ed Felten ex-White House, Series B $100M led by Lightspeed, $1.2B valuation) vs Optimism (Bedrock, fault-proof status, decentralized sequencer; Series B $150M a16z, $1.65B valuation), and the Base-OP profit split specifics (2.5% of Sequencer revenue or 15% of net profit; 118M OP governance tokens capped at 9% voting power).
...
- **"不仅仅是交易所" = a value-CREATION reframe (the differentiated angle).** The signature exchange thesis re-derives the company as MORE than a fee-taking venue: it CREATES value across multiple engines (compliance-gated asset onboarding, stablecoin distribution, staking, custody, an L2/Base ecosystem, a SuperApp) — so the equity captures value the pure trading-fee model misses. Always separate **value creation (the multi-engine platform) from value capture (which engine the stock actually monetizes, and how decorrelated from BTC price it is)** — `data-analysis.md` 2.15. This is the exchange analogue of "Ethereum ≠ ETH."
- **Two cores: 资产流量 + 合规.** Licenses = the *threshold/necessary condition*; the actual **胜负手 = new asset flow.** Compliance's essence = **吸纳传统金融机构的资产流量** (its new flow is more ToB than retail). Compliance importance is *rising* and *underestimated*. Quantify the compliance moat (`data-analysis.md` 2.9c): jurisdiction license map (US MSB+MTL, HK VASP Type1/7 + 上币须运营≥12个月, EU MiCA, SG DPT, Japan, Bermuda DAB…), full-license-set cost (≥$300万), compliance cost as % headcount (顶峰 ~1/3 员工).
...
- **Banking-revenue sub-breakdown (data anchors) + cyclicality mechanism:** 散户 55% / 机构 4% of revenue (散户占交易量 94-95%); 稳定币 ~19% / 质押 ~10% / 托管 ~2% / 利息 ~5% / 其他 ~5%. Banking业务 lowers cycle-correlation.
- **The DEFINING deliverable is a CONCRETE forward revenue FORECAST, not a paradigm essay (`data-analysis.md` 2.3b).** Build the quarterly forecast with an explicit fee-rate engine: 零售费率 ~1.7% / 机构 ~0.03% / 衍生品 ~0.03%; ETF ~90% 净流入在CB托管; USDC 利息分润 ~60% (储备 80% 国债 / 20% 现金) → e.g. 24Q1 收入 >$15亿 (model: 交易 ~$1,084M / 订阅 ~$487M = ~$1,571M), bull-peak 平均季度收入 ~$46亿, 市值 >$1000亿 (≥5x). Do not substitute a "估值范式迁移" reframe for the numeric North-Star forecast.
- **Quarterly net-income WALK proving 穿越牛熊 (not FY aggregates):** e.g. −1094 / −545 / −577 / −79 / −97 / −2 / +273 ($M), where 23Q4 = the FIRST bull-through-bear positive quarter (+$273M), driven by stablecoin+staking. FY totals (FY22 −26亿, FY24 +25.8亿) are far coarser — show the quarterly series.
- **Competitive table benchmarked as "% of Binance":** monthly head-to-head volume (e.g. Binance 现货 3月 ~$1.14万亿 vs Coinbase ~$1590亿 #4; 衍生品 Binance ~$2.3万亿 vs Coinbase国际站 ~$540亿 = ~2% of Binance) and the CEX market-share pie (Binance ~47 / OKX ~17 / Bybit ~14 / Coinbase ~2%). Give the actual 2% figure, not "个位数到十几%".
- **Base-chain metrics block (quantify it, don't treat qualitatively):** 24Q1 活跃地址 ~341k (L2 #1), 交易笔数 ~2.7m (~4x Arb), TVL ~$1.5B vs Arb ~$10B / OP ~$3.6B, 3月收入 >$2000万.
- **Cost structure + price-appreciation-vs-neutral split:** 运营 ~40-50% / 销售营销 ~10-20% / 技术开发 ~40%; 合规成本居高 (顶峰 ~1/3 员工为合规); 牌照全套 ≥$300万. Decompose transaction revenue into BTC-price-appreciation-driven vs price-neutral platform-intrinsic.
- **Platform-asset mix shift:** BTC 占比上升 (e.g. 2023Q4 BTC ~47% / ETH ~25%), 平台资产 ~$193B+, ETH 交易量比例下降 — an asset-mix observation the laggard omits.
...
- **Leverage-ratio regression → a PRICE GRID, not just a multiple (`data-analysis.md` 2.19).** Regress miner stock % change against BTC % change *separately for bull and bear* and report the slope (y = kx) per peer. Empirical anchors: MARA bull 6x (prior cycle) → ~1.5x (this cycle); CLSK ~3x; MSTR ~0.6x; bear ~1x. **Anchor the cycle start explicitly** (date + underlying price + equity price, e.g. 2022-11-21, BTC $15,776, MARA $6.19; bear max drawdown ~75% from peak as prior-cycle reference) so the regression is calibratable. Then **output the dollar grid:** bull MARA ~$38.1/$55.8/$85.2 at BTC $70K/$100K/$150K; bear ~$16.3/$20.9/$28.4 at BTC $30K/$37.5K/$50K (full upside ~8-10x from the $16K low). Miner equity has **MORE** upside elasticity than BTC in the cycle, not less — but the deliverable is the concrete price grid, not "2-3x".
- **Bottom-up cost RECONSTRUCTION, not citation (`data-analysis.md` 2.22).** Rebuild $/BTC from physics: network hashrate (~375 EH/s) + block reward + S19 XP specs (140 TH/s / 3031W / 22 J/TH / $4600 / 2.5yr life) → EH-per-unit → CapEx ~$14,300 + OpEx ~$12,200 → ~$26,500/BTC. Then rank peers by reconstructed cost (CoinShares: CLSK lowest ~$26,927 … RIOT ~$65,371) and show how many sit above breakeven.
- **Market-structure quant (size the industry, don't skip it):** top-10 miners ~20% of total hashrate; top-2 pools >51%; US+China >60% of global hashrate; global hashrate >500 EH/s (~2x YoY); the subject's own share (MARA ~3.4%). Plus the BTC ownership distribution (miners hold ~714K BTC ≈ 3.4%; Satoshi ~1.1M; individuals ~11.97M; funds/ETF ~818K) — the "miners capture little of the pie" framing.
- **Survival ranking / distance-to-bankruptcy (deliverable, not a TODO — `data-analysis.md` 2.21).** Rank peers by runway (D/E + marginal cash cost vs spot + on-BS BTC vs debt) into explicit dates (e.g. 2024.4 … 2056.1). Include the named blow-up (e.g. Core Scientific) D/E trajectory as the survival benchmark.
...
- **4-stage halving cycle model + QUANTIFIED mechanics:** The Rising Bull → Mining Goldrush → Inventory Flush → Shake Out; plus the empirical **hashrate-lags-price by 60-120 days (2-4 months)** anchor. Quantify the halving math: reward 6.25 → 3.125; post-halving BTC-production-efficiency hit ~30%; hashrate peaks ~4 months before halving then −26% to −30%. State which stage the cycle is in — the halving cycle IS the organizing framework, not a "priced-in" footnote.
- **Miner second/new revenue curve = transaction fees + 铭文 (inscriptions/Ordinals) / ecosystem — SIZE it, don't assert it.** Quantify the curve: 2023 miner fee revenue ~$797M (2nd-highest ever, after 2021 ~$1019M), ~7.6% of block reward vs ~1.5% in 2022; chart inscriptions' contribution to volume/fees. (This is the house's actual second-curve, distinct from any AI angle.)
- **Operational-asset + quarterly-financials granularity (heavy-asset business):** site-by-site EH/s (e.g. McCamey 7.1, Ellendale 7.4…), 584 MW → 910 MW capacity, fleet/per-machine profitability (S19 Pro $0.5/day vs S19 XP $4.28/day), self-hosting 3% → 45% shift, rig count 6,800 → 184,400; quarterly BTC holdings (5,129 → 15,126), production cost, gross-margin swings (86% → 52%), operating/financing cash-flow series, liquidity vs debt (e.g. $781M liquidity vs $325M debt 23Q3). Use the full series with/without depreciation ($13,262 / $36,540 / $31,342 per-cycle; marginal $16,022 vs $17,417) — not one clean pair.
...
- **Management/ownership + survival diligence with FIGURES (required α inputs — `data-analysis.md` 2.9d):** CEO background (tech operator vs capital-markets operator); insider ownership with numbers (e.g. Fred Thiel 0.14% vs Matt Schultz 1.42%; mgmt stake MARA 0.58% vs CLSK 3.46% — low = governance flag); top external holders (Vanguard 7.76%, BlackRock 5.57%); hosting model (third-party vs self-operated %) and machine-mix economics. Include a **bankruptcy case study** (e.g. Core Scientific) — contrast its D/E trajectory with the subject's.
- **Per-miner peer comparison GRID (full set, standardized multiples):** P/B, EV/Hashrate (e.g. MARA 208), EV/Sales (e.g. MARA 34.3), D/E, ranked cost-per-unit — across the full 12-14 company peer set with mktcap / share-price / YoY, not just MARA/CLSK/MSTR headline.

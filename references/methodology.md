...
- **BUILD THE FULL TABLE, all three cases incl. pessimistic (the #1 depth gap — `data-analysis.md` Part 0).** Where the house has a worked grid, reproduce every cell: the 3-case demand table (never drop the bear/pessimistic leg), the per-segment revenue+margin forecast, the per-product/per-SKU bottom-up build, the per-deal multiple table + fund IRR/DPI/RVPI ladder, the quarterly P&L walk (not FY aggregates), the DuPont time-series, the competitor market-share %s. A named framework with a vague band loses to a built table.
- **Quantify the moat and score the founder (`data-analysis.md` 2.9c/2.9d).** A moat needs a number (dev/contributor/app/download vs challenger %; license map + cost; share-of-Binance). Founder voting %, insider %, succession model, per-partner deal-share % are scored items, not footnotes.
- **Anchor relentlessly on hard ratios.** TVL %, MAU, take-rate, REV/GDP, Stable/Total Mcap, OI/Vol, DAU/MAU, penetration %, cost/tx. Prefer *ratios and shares* over absolute numbers — but still land the absolute point outputs the house produced (SOTP $, derived price).
...
5. **Output a band AND the derived point numbers**, tied to explicit *conditional* assumptions (ETH 3-5yr: $3k/$6k/$10k keyed to L1-scaling + org-efficiency + next-asset-eruption). DERIVE the price/valuation from the bottom-up worksheet (`data-analysis.md` 2.20) or leverage-proxy grid (2.19) — do not cite a round number that diverges from the table, and do not stop at "PE/G 1.2, CAGR 42%" without landing the dollar figure (SOTP $X.XXT). Any deviation from a precedent's headline must FALL OUT of the rebuilt model, not be asserted.
...
- **Hard business-anchor mandate (do the financial teardown BEFORE the framework layer).** DecadeX pairs every framework with the subject's OWN financials. Jargon-heavy, data-poor analysis is penalized. Pull the company's real numbers first: installed base / market share, segment revenue & margin, per-unit pricing, and the sector's native ratios. Worked anchors the house actually used:
  ...
  - *Funds:* a fund-by-fund **IRR/TVPI/DPI/RVPI ladder** AND a **per-deal star-case table** (named partner / entry round / entry valuation / multiple), AUM, dry powder, deal/exit counts, median deal-size & valuation trend (for the "刻舟求剑/size-moved" call), portfolio splits (stage/region/sector), per-partner deal-share % (审美趋同), the ROI-on-capital stat (<$X→$Y), and the **named blow-up**. Verify HANDS-ON vs hands-off from the named 帮得上 evidence (board control, CEO recruiting/firing), not from fund size. Do NOT invent MOIC scenarios.
  - *Conglomerates / sogo-shosha:* commit to a SINGLE focal ticker; per-company segment-revenue mix (¥+%) + overseas-revenue %; DuPont 15-20yr time-series; EV/EBITDA + dividend-yield history (not an invented P/B tree); 2-3 named quantified business-history case studies; the investor's bond-issuance amounts/dates + staged stake %s; forward CAGR + payout bands. FX is bidirectional (operating tailwind vs holder drag).
  - *Exchanges:* revenue line-item split (零售/机构/稳定币/质押/托管/利息), the fee-rate forecast engine (零售 ~1.7% / 机构 ~0.03% / ETF ~90% custody / USDC ~60%), a quarterly net-income walk (the first bull-through-bear positive quarter), Base-chain metrics, the CEX share pie and monthly volume as % of Binance, cost structure, license map + cost. Acknowledge that an EDGAR-only pass under-supplies the proprietary monthly/competitor data.
  - *Miners:* hashrate (EH/s), BTC held vs produced vs bought, avg production cost with/without depreciation (full per-period series), marginal cost, D/E, employee count, self-hosted vs third-party-hosted %, site-by-site EH/s, quarterly BTC-held/margin/cash-flow walk, market-structure quant (top-N concentration, subject's network share), a leverage-proxy PRICE GRID, and a survival/distance-to-bankruptcy RANKING. Penalize generic/forward-dated numbers.
  - *Hardware monopolists:* BOM teardown (cost vs price) and the up/down-stream squeeze (its own suppliers' pricing power + named upstream beneficiaries TSMC/HBM/ASE/optical), demand sizing in *units × $* across opt/neu/**pess** (training/inference GPU tables off cloud-capex × attach rate), CUDA-moat counts vs the challenger %, founder/governance figures, per-employee mktcap, the FULL drawdown sequence, a segment revenue+margin forecast table, and concrete point SOTP outputs ($X.XXT). Not vague capex aggregates.
- Tag claims **事实 / 推断 / 观点 (fact / inference / opinion)** when precision matters (compute-fundamentals does this per-row).
- **Anti-fabrication (`data-analysis.md` Part 0 rule 5 / `data-sources.md` rule 8).** Never invent a plausible-sounding ratio (dev ratios, fee-decline figures, mcap-% attributions) when a real source exists — pull the real figure (Electric Capital / l2beat / Token Terminal / filings) or tag it 推断/illustrative. Never attribute a fabricated metric to a prior report version. Never let an analyze.py output run on placeholder inputs anchor the headline conclusion.
